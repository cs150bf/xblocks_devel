...
if isempty(varargin) || ~ismember('-super', varargin)
    ...
    for i = 1:length(subblocks)
        if ismember('-verbose', varargin)
            [temp_list, tree_updated_branch] = get_dependlist_n_tree(subblocks{i}, depth+1, [tree_prev_branch,' <-- ', subblocks{i}], '-verbose', '-recursive');
        else
            [temp_list, tree_updated_branch] = get_dependlist_n_tree(subblocks{i}, depth+1, [tree_prev_branch,' <-- ', subblocks{i}], '-recursive');
        end
...
elseif ismember('-super', varargin) && ismember('-recursive', varargin)
    ...
    for i=1:length(recorded_subblk_names)
        ...
        if ismember(blk_name,recorded_subblk_lists{i}.(recorded_subblk_names{i}))
            ...
            if ismember('-verbose', varargin)
                [super_super_blk_names, tree_updated_branch] = get_dependlist_n_tree(super_blk_name, depth-1, [tree_prev_branch, ' --> ', super_blk_name], '-super', '-verbose', '-recursive');
            else
                [super_super_blk_names, tree_updated_branch] = get_dependlist_n_tree(super_blk_name, depth-1, [tree_prev_branch, ' --> ', super_blk_name], '-super', '-recursive');
            end
